...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'term-mode-hook (lambda ()
                                (linum-mode -1)))
#+END_SRC
** line highlight mode... disables when appropriate
#+BEGIN_SRC emacs-lisp
  ;; enable line highlight only when using gui
  (when window-system (global-hl-line-mode t))

  ;; disable line highlight in terminal emulators
  (add-hook 'eshell-mode-hook (lambda ()
...
#+BEGIN_SRC emacs-lisp
  (use-package dashboard
    ...
    (setq dashboard-banner-logo-title (shell-command-to-string "fortune -s -n 100 | tr '\n' ' ' | awk '{$1=$1;print}'")))
#+END_SRC
* Avy
...
#+END_SRC
* Beacon
A package that makes it easier to see the mark/cursor
** Initialize
#+BEGIN_SRC emacs-lisp
  (use-package beacon
    :ensure t
